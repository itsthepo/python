...
import json,random, hashlib, re, string
def randomize(data):
          faker = Faker()
          for key in data:
          ...
          # Generate if the value is a string data type but a series of numbers
              ...
              data[key] = random.randint(0,10000000)
          ...
          # Generate random user if field was flagged as user
            ...
            # a random first name and last name
              data[key] = random.choice(first_names) + random.choice(last_names)
          
          # Generate Mac Address 
            elif data[key] == "mac": 
              data[key] = "02:%02x:00:%02x:%02x:%02x" % (random.randint(0, 255),random.randint(0, 255),random.randint(0, 255),random.randint(0, 255))  
          
          # Generate File Names
            elif data[key] == "file_name":
              file_names = ["file1.txt", "file2.txt", "file3.txt", "file4.txt", "file5.txt"]
              data[key] = random.choice(file_names)

          # Generate File Paths
            elif data[key] == "file_path": 
              ...
              data[key] = random.choice([linux,windows])

          # Generate Command Line
            elif data[key] == "command_line":
              ...
              data[key] = random.choice(ps_commandline)
            
          # Generate SHA 256 Hash 
            elif data[key] == "sha_256":
              data[key] = hashlib.sha256(str(random.random()).encode("utf-8")).hexdigest()

          # Generate MD5 String
            elif data[key] == "md5":
              data[key] = hashlib.md5(str(random.random()).encode("utf-8")).hexdigest()

          # Computer names
            elif data[key] == "computer_name": 
              computer_names = ["computer1", "computer2", "computer3", "computer4", "computer5"]
              data[key] = random.choice(computer_names)
          
          # network direction
            elif data[key] == "net_dir": 
                net_dir = ["Inbound", "Outbound", "in", "out"]
                data[key] = random.choice(net_dir)
          
          # ciphers
            elif data[key] == "cipher":
              ...
              data[key] = random.choice(cipher)
          
          # emails
            elif data[key] == "emails":
              domains = ["gmail.com", "yahoo.com", "hotmail.com", "outlook.com", "aol.com", "icloud.com"]
              username = ''.join(random.choices(string.ascii_lowercase + string.digits, k=10))
              domain = random.choice(domains)
              data[key] = f"{username}@{domain}"

          # tls
            elif data[key] == "tls":
              tls = ["TLSv1.2", "TLSv1.3", "TLSv1.1", "SSLv1", "SSLv2", "SSLv3"]
              data[key] = random.choice(tls)
              
          # date
            elif data[key] == "date":
              data[key] = faker.date() 
              ## Please note, you can also specify like the commented out line before
              #data[key] = fake.date(pattern="%Y-%m-%d")
          
          # mitre_tactic
            elif data[key] == "mitre_tactic": 
              ...
              data[key] = random.choice(mitre_tactic)
            
          #mitre_technique
            elif data[key] == "mitre_technique": 
              ...
              data[key] = random.choice(mitre_technique)
          ...
          # randomizing string values BUT checks to see if the user has deemed the field static by placing ' keep | ' in front of the value you they want to save.
              ...
              if match: 
                ...
                data[key] = ''.join(random.choices(string.ascii_lowercase + string.digits, k=random.randint(min_length, max_length)))
